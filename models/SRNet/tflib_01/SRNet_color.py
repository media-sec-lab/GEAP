...
from utils_multistep_color import *


class SRNet_color(Model):
    def _build_model(self, inputs):
        ...
        with arg_scope([layers.conv2d], num_outputs=16,
            ...
            with tf.variable_scope('Layer1'): 
                ...
                # conv=layers.separable_conv2d(_inputs, num_outputs=192, kernel_size=3, depth_multiplier=1, stride=1, \
                #                              padding='SAME', data_format='NCHW')
                conv00 = layers.conv2d(_inputs[:, 0:1, :, :], num_outputs=64, kernel_size=3)
                conv01 = layers.conv2d(_inputs[:, 1:2, :, :], num_outputs=64, kernel_size=3)
                conv02 = layers.conv2d(_inputs[:, 2:, :, :], num_outputs=64, kernel_size=3)
                conv = tf.concat([conv00, conv01, conv02], 1)
            ...
            with tf.variable_scope('Layer9'):  
                ...
                convs = layers.conv2d(res, num_outputs=192, kernel_size=1, stride=2)
                convs = layers.batch_norm(convs)
                # conv1=layers.conv2d(res, num_outputs=64)
                conv1=layers.conv2d(res, num_outputs=192)
                actv1=tf.nn.relu(layers.batch_norm(conv1))
                # conv2=layers.conv2d(actv1, num_outputs=64)
                conv2=layers.conv2d(actv1, num_outputs=192)
                bn=layers.batch_norm(conv2)
                pool = layers.avg_pool2d(bn)
                res= tf.add(convs, pool)
            with tf.variable_scope('Layer10'): 
                # convs = layers.conv2d(res, num_outputs=128, kernel_size=1, stride=2)
                convs = layers.conv2d(res, num_outputs=384, kernel_size=1, stride=2)
                convs = layers.batch_norm(convs)
                # conv1=layers.conv2d(res, num_outputs=128)
                conv1=layers.conv2d(res, num_outputs=384)
                actv1=tf.nn.relu(layers.batch_norm(conv1))
                # conv2=layers.conv2d(actv1, num_outputs=128)
                conv2=layers.conv2d(actv1, num_outputs=384)
                bn=layers.batch_norm(conv2)
                pool = layers.avg_pool2d(bn)
                res= tf.add(convs, pool)
            with tf.variable_scope('Layer11'): 
                # convs = layers.conv2d(res, num_outputs=256, kernel_size=1, stride=2)
                convs = layers.conv2d(res, num_outputs=768, kernel_size=1, stride=2)
                convs = layers.batch_norm(convs)
                # conv1=layers.conv2d(res, num_outputs=256)
                conv1=layers.conv2d(res, num_outputs=768)
                actv1=tf.nn.relu(layers.batch_norm(conv1))
                # conv2=layers.conv2d(actv1, num_outputs=256)
                conv2=layers.conv2d(actv1, num_outputs=768)
                bn=layers.batch_norm(conv2)
                pool = layers.avg_pool2d(bn)
                res= tf.add(convs, pool)
            with tf.variable_scope('Layer12'): 
                # conv1=layers.conv2d(res, num_outputs=512)
                conv1=layers.conv2d(res, num_outputs=1536)
                actv1=tf.nn.relu(layers.batch_norm(conv1))
                # conv2=layers.conv2d(actv1, num_outputs=512)
                conv2=layers.conv2d(actv1, num_outputs=1536)
                bn=layers.batch_norm(conv2)
